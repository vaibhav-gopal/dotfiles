...
in {
  options.features.yabai = {
    enable = lib.mkEnableOption "enable yabai window manager for macos" // { default = false; };
    ...
    enable_full_features = lib.mkEnableOption "enable full yabai features ; BIG WARNING! Requires you to partially disable SIP (system integrity protection), enabling yabai to inject scripts into the native window manager" // { default = false; };
    config = lib.mkOption {
